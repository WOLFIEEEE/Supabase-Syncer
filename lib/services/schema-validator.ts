...
function compareConstraints(
  ...
  // Check CHECK constraints in target (filter out auto-generated NOT NULL constraints)
  const targetChecks = targetTable.constraints.filter((c) => c.type === 'CHECK');
  
  // Filter out Supabase/PostgreSQL auto-generated NOT NULL constraints
  // These have names like "2200_18331_1_not_null" or end with "_not_null"
  const meaningfulChecks = targetChecks.filter((check) => {
    const name = check.name.toLowerCase();
    // Skip auto-generated NOT NULL constraints
    if (name.endsWith('_not_null')) return false;
    // Skip internal PostgreSQL constraints with numeric OID patterns
    if (/^\d+_\d+_\d+_/.test(name)) return false;
    return true;
  });
  
  // Only report meaningful CHECK constraints, not auto-generated ones
  if (meaningfulChecks.length > 0) {
    for (const check of meaningfulChecks) {
      issues.push({
        id: generateIssueId(),
        severity: 'INFO',
        category: 'Constraints',
  ...
  // Summarize auto-generated NOT NULL constraints if there are many
  const autoGeneratedCount = targetChecks.length - meaningfulChecks.length;
  if (autoGeneratedCount > 0) {
    constraintIssues.push(`${autoGeneratedCount} auto-generated NOT NULL constraint(s)`);
  }
