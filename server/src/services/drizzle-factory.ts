 ...
 * 
 * Creates database connections for sync operations
 */
...
export async function executeWithTimeout<T>(
  _connection: DrizzleConnection, // Connection passed for type checking, query closure uses it
  query: () => Promise<T>,
...
export async function testConnection(databaseUrl: string): Promise<{ success: true; version: string; tableCount: number } | { success: false; error: string }> {
  ...
  try {
    ...
    logger.error({ error, url: maskDatabaseUrlForLogs(databaseUrl) }, 'Connection test failed');
...
export async function getTableRowCount(databaseUrl: string, tableName: string): Promise<number> {
  ...
  try {
    ...
    logger.error({ error, tableName }, 'Error getting row count');
    return 0;
  } finally {
    if (connection) {
      await connection.close();
